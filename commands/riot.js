...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (interaction.channelId !== config.riotChannel) {
            const wrongChannelEmbed = new MessageEmbed()
                .setColor(config.color.red)
                .setTitle('Wrong command usage!')
                .setDescription(`You can only use the \`/riot\` command in <#${config.riotChannel}> channel!`)
        ...
        const filePath = `${__dirname}/../riot/${service}.txt`;
        ...
        fs.readFile(filePath, 'utf-8', (error, data) => {
            ...
            fs.writeFile(filePath, updatedData, (writeError) => {
                ...
                const embedMessage = new MessageEmbed()
                    ...
                    .setTitle('Generated Riot Account')
                    .setFooter(interaction.user.tag, interaction.user.displayAvatarURL({ dynamic: true, size: 64 }))
                    .setDescription('🙏 Thank you so much for being a KING member! \n 🌟 Your support means the world to us! 💖😊')
